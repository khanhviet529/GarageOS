...
describe('🔒 Quét: dữ liệu chi nhánh khác không lọt ra endpoint nào', () => {
  ...
  test('🔒 hàng rào: mọi route @Get trong mã nguồn đều nằm trong danh sách quét', () => {
    ...
    const boQua: Record<string, string> = {
      ...
      'api/v1/admin/users': 'danh bạ người dùng của tenant — không phải dữ liệu của một chi nhánh',
      ...
      'api/v1/public/redirect': 'tra chuyển hướng công khai, tenant lấy từ hostname',
      'api/v1/marketing/articles': 'bài viết là nội dung của cả tenant — bảng chỉ có tenant_id',
      'api/v1/marketing/articles/:id/draft': 'bản nháp của bài viết, phạm vi tenant',
      'api/v1/marketing/article-categories': 'chuyên mục bài viết, phạm vi tenant',
      'api/v1/public/articles': 'bài viết công khai, tenant lấy từ hostname',
      'api/v1/public/articles/:slug': 'bài viết công khai, tenant lấy từ hostname',
